...
def folderStructure():
    ...
    fmap.write("name\tcik\tyear\tmonth\tday\tnr\n")
    ...
    for f in listdir(fullpath):
        ...
        fmap.write("{0}\t {1}\t {2}\t {3}\t {4}\t {5}\n".format(f,fAux[0],fAux[1],fAux[2],fAux[3],fAux[4]))
...
def trainDoc2Vec(docs, year):
    ...
    model = doc2vec.Doc2Vec(size=300, window=100, min_count=2, iter=30, workers=8, dm=1, negative=10)
